...
create policy services_admin_all on services for all using (current_setting('jwt.claims.role', true) = 'admin');
...
create policy appointments_staff_select on appointments
  for select to authenticated using (current_setting('jwt.claims.role', true) in ('admin','staff'));

create policy appointments_staff_update on appointments
  for update to authenticated using (current_setting('jwt.claims.role', true) in ('admin','staff'))
  with check (current_setting('jwt.claims.role', true) in ('admin','staff'));

create policy appointments_admin_delete on appointments
  for delete to authenticated using (current_setting('jwt.claims.role', true) = 'admin');
...
create policy bh_admin_all on business_hours for all using (current_setting('jwt.claims.role', true) = 'admin');
...
create policy blocked_admin_all on blocked_dates for all using (current_setting('jwt.claims.role', true) = 'admin');
